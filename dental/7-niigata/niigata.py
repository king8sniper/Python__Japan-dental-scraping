...
base_url = 'http://qq.niigata-iyaku.jp/qq15/qqport/kenmintop/detail/fk1102.php?kinouid=fk9117&sisetuid='
...
def get_base_data(detailHtml):
    ...
    try:
        oral_surgery = "["
        general_tds = html_info.find('table', {'summary': '歯科口腔外科領域一覧'}).find_all('td')
        for general_td in general_tds:
    ...
    try:
        affiliate_check = "["
        general_tds = html_info.find('table', {'summary': '連携の有無'}).find_all('td')
        for general_td in general_tds:
            if general_td.text.strip() == "実施":
    ...
    try:
        ...
        dental_hygienist = "na"
    try:
        average_people_count = html_info.find('table', {'summary': '患者数'}).find('th', string='前年度１日平均患者数').find_next_sibling('td').text.strip()
    except:
    ...
    medical_department = '['
    try:
        medical_department += (html_info.find('caption', string='歯科領域').text.strip() + ", ")
    except:
        print('not')
    try:
        medical_department += html_info.find('caption', string='歯科口腔外科領域').text.strip()
    except:
        print('not')
    medical_department += ']'
...
def init():
    ...
    csv_file_name = "niigata" + str(Today) + ".csv"
