...
def get_showcase_info(showcaseHtml):
    ...
    try:
        general_tds = html_info.find("th", string="前年度１日平均患者数").find_next_siblings('td')
        if len(general_tds) == 1:
            average_people_count = general_tds[0].text.strip().replace("　", "").replace("\n", "")
        else:
            average_people_count = general_tds[5].text.strip().replace("　", "").replace("\n", "")
    except:
