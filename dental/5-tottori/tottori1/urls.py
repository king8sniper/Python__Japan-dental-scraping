...
def get_urls(driver, total_page):
    for page in range(1, total_page + 1):
        ...
        table_element = driver.find_element(By.CSS_SELECTOR, "table[class='list_table']")
        tr_elements = table_element.find_elements(By.TAG_NAME, 'tr')
        # print(a_elements)
        for tr_el in tr_elements[1:]:
            a_el = tr_el.find_elements(By.TAG_NAME, 'a')[0]
            url = a_el.get_attribute('href')
            print(url)
            url_write(url, page)
            
        if len(driver.find_elements(By.XPATH, "//a[text()='次の50件へ']")) > 0:
            btn_next = driver.find_element(By.XPATH, "//a[text()='次の50件へ']") 
            btn_next.click()
        else :
            driver.close()
