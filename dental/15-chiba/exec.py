...
fc = open('chiba.csv', 'a', newline='', encoding='utf-8')
...
def get_base_data(html):
    ...
    print('baseData')
    baseData['updated_at'] = html.find_element(By.ID, 'ctl00_ContentPlaceHolderContents_lblLastUpdateDate').text.split('最終報告日')[1] if '最終報告日' in html.find_element(By.ID, 'ctl00_ContentPlaceHolderContents_lblLastUpdateDate').text else html.find_element(By.ID, 'ctl00_ContentPlaceHolderContents_lblLastUpdateDate').text if len(html.find_elements(By.ID, 'ctl00_ContentPlaceHolderContents_lblLastUpdateDate')) > 0 else 'na'
...
def get_result_data(html):
    table_element=html.find_elements(By.XPATH,"//table[@class='DetailTable']")[1]
    day_patients_td=table_element.find_element(By.XPATH, "//td[@class='DetailTitle' and contains(text(), '日当たりの患者数')]")
