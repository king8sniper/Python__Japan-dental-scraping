...
base_url = 'http://www.qq.pref.shizuoka.jp/qq22/qqport/kenmintop/detail/fk1102.php?kinouid=fk9020&sisetuid='
...
def get_base_data(detailHtml):
    ...
    try:
        storename = html_info.find('table', {'summary': '歯科診療所の正式名称、通称略称一覧'}).find_all('tr')[1].find('td').text.strip()
    except:
        storename = "na"
    try:
        address_original = html_info.find('table', {'summary': '歯科診療所の所在地詳細'}).find_all('tr')[2].find('td').text.strip().replace("　"," ").replace("静岡県","")
    except:
        address_original = "na"
    try:
    ...
    try:
        founder_text = html_info.find('table', {'summary': '歯科診療所の開設者名'}).find_all("tr")[2].find('td').text.strip()
        ...
        founder_name = "na"
    try:
        admin_name = html_info.find('table', {'summary': '歯科診療所の管理者'}).find_all("tr")[1].find('td').text.strip()
    except:
        admin_name = "na"
    try:
    ...
    try:
        general_dentistry = "["
        general_tds = html_info.find('table', {'summary': '歯科一覧'}).find_all('td')
    ...
    try:
        oral_surgery = "["
        general_tds = html_info.find('table', {'summary': '歯科口腔外科一覧'}).find_all('td')
        for general_td in general_tds:
    ...
    try:
        has_structure = "["
        general_td = html_info.find('table', {'summary': '歯科診療所の駐車場の有無'}).find_all('tr')[0].find('td')
    ...
    try:
        affiliate_check = "["
        general_tds = html_info.find('table', {'summary': '他施設との連携'}).find_all('td')
        for general_td in general_tds:
            if general_td.text.strip() == "実施":
    ...
    try:
        general_th = html_info.find('h5', {'id': 'subject01f'}).find_next_sibling('p').find_next_sibling('table').find('th', string='歯科医師')
        ...
        dentist = "na"
    try:
        general_th = html_info.find('h5', {'id': 'subject01f'}).find_next_sibling('p').find_next_sibling('table').find('th', string='歯科衛生士')
        total = general_th.find_next_sibling('td').text.strip()
        full = general_th.find_next_sibling('td').find_next_sibling('td').text.strip()
        part = general_th.find_next_sibling('td').find_next_sibling('td').find_next_sibling('td').text.strip()
        ...
        dental_hygienist = "na"
    try:
        average_people_count = html_info.find('table', {'summary': '患者数'}).find('th', string='前年度一日平均外来患者数').find_next_sibling('td').text.strip()
    except:
...
def init():
    ...
    csv_file_name = "shizuoka" + str(Today) + ".csv"
